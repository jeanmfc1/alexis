...
def main():
    ...
    print(f"\n{'='*W}")
    print(f"  BUILD MASTER DB FROM AACT")
    ...
    # ── Steps 3-4: Assemble + classify all trials ─────────────────
    print(f"\n{'─'*W}")
    print(f"  STEPS 3-4: Assembling & classifying {total_ncts:,} trials")
    print(f"{'─'*W}")
    ...
    print(f"  Assembled: {len(trials):,} ({total_skipped} skipped)")
    ...
    print(f"  Classified: {elapsed_cls:.1f}s ({per_sec:.0f}/sec)")

    del trials
    gc.collect()

    trials = [t for t in classified if t.is_drug_trial]
    del classified
    gc.collect()
