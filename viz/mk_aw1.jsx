// -----------------------------------------------------------------------
// viz/mk_aw1.jsx
// Marketing / aw2 (ANZCTR) -- TA Momentum Stat Cards (monthly cadence)
//
// Data source : ALEXIS_DATA.aw2  (cards + items from
//               analytics/mk_aw1.py::aw2_anzctr_ta_momentum)
// Globals used from host: modColor, humanMod
// -----------------------------------------------------------------------

function AW2AnzctrTAMomentum({ data, color }) {
  if (!data || data.available === false) {
    return (
      <div style={{ padding:"28px 24px", fontFamily:"var(--fm)",
        ...
        {data?.reason || "No ANZCTR momentum data yet. Run pipelines/diff_anzctr_snapshots.py."}
  ...
  return (
    <div>
      ...
      <div style={{ padding:"6px 12px 10px", fontFamily:"var(--fm)",
        ...
        &nbsp;= this period (monthly) {">="} {th.accelerating_ratio}x baseline AND {">="} {th.accelerating_min} trials&nbsp;
      ...
      <div style={{ marginTop:18, padding:"0 12px" }}>
        <div style={{ fontFamily:"var(--fm)", fontSize:9,
          ...
          All TAs -- this period (monthly) vs baseline
        </div>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse",
            ...
            <thead>
              <tr style={{ background:"var(--surf2)" }}>
                {["TA","This period","Baseline","Ratio","Trend","Top modality"]
                  .map(h => (
            ...
            <tbody>
              {items.slice(0, 25).map((r, i) => {
                ...
                const periodVal = r.this_period ?? r.this_week ?? 0;
                return (
                  <tr key={i} style={{
        ...
        <div style={{ marginTop:8, fontFamily:"var(--fm)", fontSize:9,
          color:"var(--dim)", letterSpacing:"0.06em" }}>
          Baseline = monthly-normalised mean of{" "}
          {data.pair_windows || 0} valid consecutive prior-snapshot delta(s).
          Snapshots used: {data.snapshots_used}.
          {data.pair_windows_skipped ? (
            <React.Fragment>
              {" "}&middot; Skipped {data.pair_windows_skipped} pair(s) as likely backfill
              (spaced &lt; 3 days or &gt; 500 drug-trials/day).
            </React.Fragment>
          ) : null}
        </div>
      </div>
