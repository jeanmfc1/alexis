...
//
// Mirrors sci_wq1.jsx interactivity: heat-coloured bubbles sized by count,
// click to drill into trials, phase chips, row/column totals.
//
// Data source : ALEXIS_DATA.cw3  (matrix from analytics/sci_cw1.py)
...
function CW3ChinaTAModalityMatrix({ data, color }) {
  if (!data || !data.available) {
    return (
      <div style={{ padding:"24px 16px", fontFamily:"var(--fm)",
        fontSize:12, color:"var(--muted)", textAlign:"center" }}>
        No new Chinese drug trials this week.
      </div>
    );
  }
  ...
  const header = (
    <div style={{ padding:"2px 14px 10px", display:"flex",
      ...
      <span>
        ...
        &nbsp;new drug trials this week
      </span>
      {data.has_heat && (
        <>
          <span style={{ color:"var(--dim)" }}>|</span>
          <span>
            heat vs {data.prior_weeks_used}-week avg:
          </span>
            ...
            <div style={{ display:"flex", paddingLeft:LABEL_W }}>
              {cols.map(m => (
                <div key={m} style={{ width:COL_W, textAlign:"center",
                  ...
                  {CW3_MOD_ABBR[m] || humanMod(m).slice(0, 5)}
                </div>
              ))}
            ...
            {rows.map((ta, ri) => (
              <div key={ta} style={{
                ...
                {cols.map(m => {
                  ...
                  const hc     = cw3HeatToColor(cell.heat);
                  return (
          ...
          <div style={{ width:360, maxHeight:580, overflowY:"auto",
            ...
            <div style={{ display:"flex", flexDirection:"column",
              ...
              {(activeCell.trials || []).map(t => {
                ...
                const pc    = CW3_PHASE_COLORS[phase] || "#4B5563";
                return (
