"""
analytics/sci_cw1.py
---------------------
Scientific / cw3 (ChiCTR) -- TA x Modality Bubble Matrix

Mirrors the output shape of analytics/sci_wq1.py (wq7_ta_modality_matrix)
so the existing bubble-matrix JSX can render China data identically.

Provenance note:
    Labels are ML-assigned (intl_drug_classifier_v2 + intl_ta_classifier).
    No MeSH ancestry available for ChiCTR.
"""
...
def cw3_china_ta_modality_matrix(enriched_trials, prior_snapshots=None):
    """
    Args:
        enriched_trials: trials list from chictr_enriched_*.json
        prior_snapshots: optional list of parsed prior ChiCTR snapshot dicts
                         (used to compute per-cell heat via week-over-week rate)

    Returns a dict matching sci_wq1 / WQ7BubbleMatrix consumers.
    """
    # 1. Count this-week new drug registrations by TA x modality
    ...
    # 3. Compute per-cell baseline and heat from prior snapshots via
    #    consecutive-pair set differences (same logic as mk_cw1 baseline).
    prior_weeks = []
    if prior_snapshots and len(prior_snapshots) >= 2:
        ...
        for older, newer in zip(snaps_asc, snaps_asc[1:]):
            ...
            for t in pw_new:
                pw_ta_mod[t.get("therapeutic_area") or "Unknown"][
                    t.get("modality") or "Unknown"
                ] += 1
            # Window label = as_of of newer snapshot, minus older as_of
    ...
    # Per-cell prior-week avg proportion (for heat)
    prior_avg_pct = {}
    if has_heat:
        for ta in top_tas:
            for mod in top_mods:
