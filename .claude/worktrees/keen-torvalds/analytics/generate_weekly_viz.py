...
def pick_master_db() -> tuple:
    """
    Find the latest master_DB_*.json in MASTER_DB_DIR, present it to the
    user, and return its summary{} block and trials[] list.

    Returns (summary_dict, trials_list, filename) — or ({}, [], "") if
    not found / skipped.
    """
    print("\n  \u250c\u2500 MASTER DB SELECTION \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500")
    ...
    try:
        ...
        return summary, trials, chosen.name
    except Exception as e:
...
def main():
    ...
    master_summary, master_db_trials, master_db_filename = pick_master_db()
    ...
    # Quarterly questions
    master_db_path = MASTER_DB_DIR / master_db_filename if master_db_filename else None
    if master_db_path and master_db_path.exists():
        sq9_data = sq9_confidence_dashboard(str(master_db_path))
    else:
        sq9_data = {"available": False}
    if sq9_data.get("available"):
        mc = sq9_data["modality_confidence"]
        tc = sq9_data["ta_confidence"]
        dc = sq9_data["data_coverage"]
        print(f"  sq9 : modality {mc['score']:.1f}% TA {tc['score']:.1f}% "
              f"({dc['total_drug_trials']:,} drug trials, "
              f"MeSH coverage: mod {dc['modality_mesh_coverage_pct']:.0f}% "
              f"TA {dc['ta_mesh_coverage_pct']:.0f}%)")
    else:
        print("  sq9 : no data — master DB not loaded or empty")
