  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div>
          ...
          <StackedBar height={16} total={dc.total_drug_trials} segments={[
            { label: "MeSH available", value: dc.modality_mesh_available, color: TIER_COLORS.high },
        ...
        <div>
          ...
          <StackedBar height={16} total={dc.total_drug_trials} segments={[
            { label: "MeSH available", value: dc.ta_mesh_available, color: TIER_COLORS.high },
      ...
      <StackedBar height={22} total={mc.denominator} segments={[
        { label: "High (MeSH tree)", value: mc.high_count, color: TIER_COLORS.high },
        { label: "Medium (text / type)", value: mc.medium_count, color: TIER_COLORS.medium },
      ]} />
      ...
      <StackedBar height={22} total={tc.denominator} segments={[
        { label: "High (MeSH evidence)", value: tc.high_count, color: TIER_COLORS.high },
        { label: "Low (text / fallback)", value: tc.low_count, color: TIER_COLORS.low },
      ]} />
              ...
              return (
                <div key={flag} style={{ display: "grid", gridTemplateColumns: "1fr 80px",
                  ...
                  <div style={{ fontFamily: "var(--fm)", fontSize: 11, fontWeight: 600,
                    color: TIER_COLORS.medium, textAlign: "right" }}>{fmt(count)}</div>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        {perTa.map(row => {
          const modColor_ = row.mod_high_pct >= 80 ? TIER_COLORS.high
            : row.mod_high_pct >= 60 ? TIER_COLORS.medium : TIER_COLORS.low;
          const taColor = row.ta_high_pct >= 80 ? TIER_COLORS.high
            : row.ta_high_pct >= 60 ? TIER_COLORS.medium : TIER_COLORS.low;
          return (
            <div key={row.ta} style={{ display: "grid",
              ...
              <div style={{ display: "flex", height: 12, borderRadius: 3, overflow: "hidden",
                background: "var(--surf2)" }}>
                <div style={{ width: `${row.mod_mesh_pct}%`, background: TIER_COLORS.high, opacity: 0.5 }} />
              </div>
          ...
          <tbody>
            {sortedPriorities.map((row, i) => (
              <tr key={i} style={{ background: i % 2 === 0 ? "transparent" : "var(--surf2)" }}>
                ...
                <td style={{ ...tdStyle, color: TIER_COLORS.low }}>{fmt(row.low_count)}</td>
                <td style={tdStyle}>
                  <span style={{ background: TIER_COLORS.low + (Math.round(row.pct_low / 100 * 40 + 10).toString(16).padStart(2, "0")),
