...
_MATURITY_LABELS = {
    "P1": "FIH-heavy",
    "P2": "Dose finding-heavy",
    "P3": "Comparative-heavy",
    "P4": "RWE-heavy",
}


def _phase_buckets(phase_dist: dict) -> dict:
    """Bucket raw phase keys into P1/P2/P3/P4."""
    buckets = {"P1": 0, "P2": 0, "P3": 0, "P4": 0}
    for k, v in phase_dist.items():
        ku = k.upper()
        if "PHASE2/PHASE3" in ku:
            buckets["P2"] += v
        elif any(x in ku for x in ("EARLY", "PHASE1")) and "PHASE2" not in ku:
            buckets["P1"] += v
        elif "PHASE2" in ku and "PHASE3" not in ku:
            buckets["P2"] += v
        elif "PHASE3" in ku:
            buckets["P3"] += v
        elif "PHASE4" in ku:
...
def _maturity_label(phase_dist: dict) -> tuple[str, float]:
    """Return (label, dominant_pct) for phase distribution."""
    total = sum(phase_dist.values())
    if total == 0:
        return "balanced", 0.0
    buckets = _phase_buckets(phase_dist)
    dominant = max(buckets, key=buckets.get)
    dom_pct = round(buckets[dominant] / total * 100, 1)
    if dom_pct > 30:
        return _MATURITY_LABELS[dominant], dom_pct
    return "balanced", dom_pct
...
def sq1_modality_landscape(master_db_path: str) -> dict:
    ...
    for mod_name, mod_trials in by_mod.items():
        ...
        maturity, maturity_dom_pct = _maturity_label(phase_dist)
        ...
        mod_rows.append({
            ...
            "maturity":            maturity,
            "maturity_dominant_pct": maturity_dom_pct,
